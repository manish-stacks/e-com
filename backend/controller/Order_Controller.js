...
exports.verifyCODFee = async (req, res) => {
  try {
    ...
    const redirectUrl = `https://grandmasala.in/receipt-cod/order-confirmed?id=${order._id}&success=true&type=cod&amountToPay=${order.totalAmount}`;
...
exports.checkStatus = async (req, res) => {
  try {
    ...
    if (!isValid) {
      ...
      const failureRedirect = `https://grandmasala.in/payment-failed?order_id=${razorpay_order_id}`;
    ...
    const successRedirect = `https://grandmasala.in/Receipt/order-confirmed?id=${findOrder._id}&success=true&data=${findOrder?.orderId}`;
