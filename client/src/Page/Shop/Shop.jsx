...
const Shop = () => {
  ...
  const transformProduct = (apiProduct) => {
    const getLowestPrice = () => {
      if (
        apiProduct.isVarient &&
        apiProduct.Varient &&
        apiProduct.Varient.length > 0
      ) {
        return Math.min(
          ...apiProduct.Varient.map((v) => v.price_after_discount || v.price)
        );
      }
  ...
  const handleSearchAndFilter = async (query = "", page = 1) => {
    try {
      ...
      let url = "https://api.grandmasala.in/api/v1/";
      if (query && query.trim()) {
        url += `search_product_and_filter?query=${encodeURIComponent(
          query.trim()
        )}&page=${page}`;
      } else {
      ...
      if (data.success) {
        const transformedProducts = (data.products || data.data || []).map(
          transformProduct
        );
    ...
    if (priceRange !== "all") {
      ...
      filtered = filtered.filter((product) => {
        const currentPrice = selectedVariants[product.id]
          ? selectedVariants[product.id].price_after_discount ||
      ...
      case "price-low":
        filtered.sort((a, b) => {
          const priceA = selectedVariants[a.id]
            ? selectedVariants[a.id].price_after_discount ||
            selectedVariants[a.id].price
            : a.price;
          const priceB = selectedVariants[b.id]
            ? selectedVariants[b.id].price_after_discount ||
            selectedVariants[b.id].price
            : b.price;
  ...
  const ProductCard = ({ product }) => {
    ...
    return (
      <div className="group container relative bg-white rounded-2xl shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden border border-gray-100 hover:border-gray-200">
        {/* Product Image */}
        <div className="relative container overflow-hidden bg-gray-50 rounded-t-2xl">
          <img
            src={product.image}
            alt={product.name}
        ...
        <div className="p-6 space-y-4">
          ...
          <Link
            to={`/product-page/${product.id}`}
            className={`w-full py-3 rounded-xl font-medium transition-all duration-300 flex items-center justify-center gap-2 group-hover:scale-105 ${inStock
      ...
      <div className="min-h-screen py-12">
        <div className="container mx-auto px-6">
          <div className="text-center">
            <div className="animate-spin rounded-full h-32 w-32 border-b-2 border-[#81190B] mx-auto"></div>
      ...
      >
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          {/* <p className="text-xl md:text-2xl text-gray-200 max-w-3xl mx-auto leading-relaxed">
            Discover the finest 100% natural spices.
          </p> */}
        </div>
      </div>
      ...
      <div className="min-h-screen  py-12">
        <div className="mx-auto px-6 container">
          ...
          {/* <div className="text-center mb-12">
                    <h1 className="text-4xl lg:text-5xl font-black text-[#81190B] mb-4">
                        Our Shop
                    </h1>
            ...
            <div className="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[#81190B] text-sm">
                    Found {filteredProducts.length} products
                  </p>
                </div>
                <button
                  onClick={clearSearch}
                  className="text-[#81190B] hover:text-blue-800 font-medium text-sm underline"
                >
                  Clear search
                </button>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <option value="0-500">Under ₹100</option>
                    <option value="500-1000">₹100 - ₹200</option>
                    <option value="500-1000">Under ₹500</option>
          ...
          {filteredProducts.length === 0 && !loading && (
            <div className="text-center py-16">
              ...
              {searchTerm && (
                <button
                  onClick={clearSearch}
                  className="mt-4 px-6 py-2 bg-[#010101] text-white rounded-lg hover:bg-[#81190B] transition-colors"
                >
                  Show All Products
                </button>
              )}
